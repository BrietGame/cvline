...
WHERE {$wpdb->prefix}pivot_usertocv.user_id = $userId",
    OBJECT
);
...
WHERE {$wpdb->prefix}pivot_work.cv_id = $getUserCv",
        OBJECT
    );
...
WHERE {$wpdb->prefix}pivot_skills.cv_id = $getUserCv",
        OBJECT
    );
...
WHERE {$wpdb->prefix}pivot_hobbie.cv_id = $getUserCv",
        OBJECT
    );
...
WHERE {$wpdb->prefix}pivot_school.cv_id = $getUserCv",
        OBJECT
    );
